...
XENG1 = np.array(
    [0.0, .001, .002, .003, .004, .005, .006, .007, .008, .009, 0.01, .012, .014, .016, .018, 0.02, .025, 0.03, .035,
     ...
     4.0e8, 4.5e8, 5.0e8, 6.0e8, 7.0e8, 8.0e8, 9.0e8, 1.0e9],dtype=np.float32)
gd['gas1/XEN'] = XENG1
...
YELTG1 = np.array(
    [12.5, 8.70, 7.00, 5.95, 5.20, 4.70, 4.30, 3.95, 3.65, 3.40, 3.20, 2.85, 2.58, 2.37, 2.19, 2.04, 1.77, 1.57, 1.41,
     ...
     .0140, .0140, .0139, .0139, .0139, .0139, .0139, .01389, .01389, .01389],dtype=np.float32)
for i in range(0, 26):
...
# EPSILON =1-YEPS
YEPSG1 = np.array([1.0],dtype =np.float32)
for i in range(0, 39):
    YEPSG1=np.append(YEPSG1,1.0)
YEPSG1 = np.append(YEPSG1,
    np.array([.68056, .73101, .76161, .70664, .61270, .53797, .37888, .30604, .35505, .40830, .42979, .41826, .39140, .40794,
     ...
     4.37e-10, 2.98e-10, 2.16e-10, 1.63e-10, 1.28e-10, 1.02e-10, 7.00e-11, 5.10e-11, 3.80e-11, 3.00e-11, 2.40e-11],dtype=np.float32))
gd['gas1/YEPS'] = YEPSG1

# VIBRATION V4 (RESONANCE ONLY) SCALED BY 1/E**3 ABOVE 50EV
XVBV4G1 = np.array([0.0783, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.0, 15.0, 20.0, 50.0],dtype=np.float32)
YVBV4G1 = np.array([0.0, 0.0, 0.05, 0.35, 1.06, 1.40, 1.26, 0.97, 0.07, .022, 1.e-3],dtype=np.float32)
gd['gas1/XVBV4'] = XVBV4G1
gd['gas1/YVBV4'] = YVBV4G1

# VIBRATION V1 (RESONANCE ONLY) SCALED BY 1/E**3 ABOVE 50EV
XVBV1G1 = np.array([0.1126, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.0, 15.0, 20.0, 50.0],dtype=np.float32)
YVBV1G1 = np.array([0.0, 0.0, .016, .118, 0.36, 0.47, 0.42, 0.33, .023, .007, 3.e-4],dtype=np.float32)
gd['gas1/XVBV1'] = XVBV1G1
gd['gas1/YVBV1'] = YVBV1G1

# VIBRATION V3 (RESONANCE ONLY) SCALED BY 1/E**3 ABOVE 50EV
XVBV3G1 = np.array([0.1588, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.0, 15.0, 20.0, 50.0],dtype=np.float32)
YVBV3G1 = np.array([0.0, 0.0, 0.15, 1.05, 3.19, 4.20, 3.78, 2.90, 0.20, .067, 3.e-3],dtype=np.float32)
gd['gas1/XVBV3'] = XVBV3G1
gd['gas1/YVBV3'] = YVBV3G1

# VIBRATION HARMONIC 2(V3) SCALED BY 1/E ABOVE 50EV
XVIB5G1 = np.array([0.3176, 1.00, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.0, 15.0, 20.0, 50.0],dtype=np.float32)
YVIB5G1 = np.array([0.0, .001, 0.01, .031, 0.23, 0.67, 0.87, 0.79, 0.60, .042, .014, .0006],dtype=np.float32)
gd['gas1/XVIB5'] = XVIB5G1
gd['gas1/YVIB5'] = YVIB5G1

# VIBRATION HARMONIC (3(V3) + ALL OTHER HARMONICS)
# SCALED BY 1/E ABOVE 50 EV
XVIB6G1 = np.array([0.4764, 1.00, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.0, 15.0, 20.0, 50.0],dtype=np.float32)
YVIB6G1 = np.array([0.0, .0009, .045, .117, .774, 2.32, 3.06, 2.75, 2.12, .138, .037, .0018],dtype=np.float32)
...
XCF2G1 = np.array(
    [21.47, 24.0, 26.0, 28.0, 30.0, 35.0, 40.0, 45.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200.,
     250., 300., 400., 500., 600., 800., 1000., 1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCF2G1 = np.array(
    [0.00, .003, .010, .032, .060, .131, .148, .162, .192, .221, .234, .243, .256, .263, .266, .260, .257, .240,
     .233, .212, .186, .169, .152, .131, .113, .0961, .0834, .0763, .0681, .0585, .0503],dtype=np.float32)
gd['gas1/XCF2'] = XCF2G1
gd['gas1/YCF2'] = YCF2G1
...
XCF1G1 = np.array(
    [29.14, 30.0, 35.0, 40.0, 45.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200., 250., 300., 400.,
     500., 600., 800., 1000., 1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCF1G1 = np.array(
    [0.00, .0024, .0145, .0583, .107, .156, .185, .226, .239, .238, .266, .274, .259, .261, .234, .227, .186, .146,
     .122, .113, .0909, .0820, .0695, .0603, .0552, .0493, .0423, .0364],dtype=np.float32)
gd['gas1/XCF1'] = XCF1G1
gd['gas1/YCF1'] = YCF1G1
...
XCF32G1 = np.array(
    [41.0, 45.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200., 250., 300., 400., 500., 600., 800.,
     1000., 1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCF32G1 = np.array(
    [0.00, .0053, .0083, .0104, .0135, .0154, .0164, .0187, .0208, .0198, .0208, .0198, .0187, .0167, .0135,
     .0114, .0101, .0079, .0065, .0055, .0048, .0044, .0039, .0033, .0029],dtype=np.float32)
...
XCF0G1 = np.array(
    [34.77, 35.0, 40.0, 45.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200., 250., 300., 400., 500.,
     600., 800., 1000., 1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCF0G1 = np.array(
    [0.0, .0005, .0093, .0426, .0884, .134, .172, .193, .207, .228, .245, .246, .249, .236, .222, .191, .166, .144,
     .134, .104, .0895, .0759, .0658, .0602, .0538, .0462, .0397],dtype=np.float32)
gd['gas1/XCF0'] = XCF0G1
gd['gas1/YCF0'] = YCF0G1
...
YC0FG1 = np.array([
    ...
    .227, .174, .170, .144, .125, .114, .102, .0874, .0753],dtype=np.float32)
gd['gas1/XC0F'] = XC0FG1
gd['gas1/YC0F'] = YC0FG1
...
YCF22G1 = np.array(
    [0.0, .0002, .0033, .0095, .0194, .0287, .0348, .0409, .0483, .0521, .0522, .0517, .0467, .0458, .0367, .0303,
     .0280, .0218, .0164, .0139, .0120, .0110, .0098, .0084, .0073],dtype=np.float32)
...
XCFFG1 = np.array(
    [43.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200., 250., 300., 400., 500., 600., 800., 1000.,
     1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCFFG1 = np.array(
    [0.0, .001, .009, .028, .049, .077, .084, .111, .125, .136, .139, .126, .133, .109, .095, .078, .059, .040,
     .034, .030, .027, .024, .021, .018],dtype=np.float32)
gd['gas1/XCFF'] = XCFFG1
gd['gas1/YCFF'] = YCFFG1
...
YCF2FG1 = np.array(
    [0.0, .001, .004, .013, .024, .034, .043, .046, .053, .054, .057, .056, .049, .050, .042, .036, .030, .023,
     .015, .013, .011, .0104, .0093, .0080, .0069],dtype=np.float32)
...
XCF3FG1 = np.array(
    [36.0, 40.0, 45.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100., 120., 140., 160., 200., 250., 300., 400., 500., 600.,
     800., 1000., 1250., 1500., 1750., 2000., 2500., 3000.],dtype=np.float32)
YCF3FG1 = np.array(
    [0.0, .001, .003, .006, .014, .023, .030, .037, .038, .042, .041, .044, .045, .040, .038, .033, .028, .023,
     .018, .012, .0105, .0091, .0083, .0074, .0064, .0055],dtype=np.float32)
...
XKSHCG1 = np.array(
    [285., 298., 307., 316., 325., 335., 345., 365., 398., 422., 447., 473., 501., 531., 613., 668., 708., 750.,
     ...
     4.10e8, 5.01e8, 6.13e8, 7.08e8, 8.18e8, 1.0e9],dtype=np.float32)
YKSHCG1 = np.array(
    [0.00, 1.66e-4, 3.48e-4, 5.25e-4, 6.96e-4, 8.63e-4, 1.02e-3, 1.33e-3, 1.75e-3, 2.01e-3, 2.24e-3, 2.46e-3,
     ...
     1.74e-4, 1.80e-4, 1.87e-4, 1.92e-4, 1.96e-4, 2.00e-4, 2.02e-4, 2.05e-4, 2.09e-4],dtype=np.float32)
...
XKSHFG1 = np.array(
    [685.4, 705., 726., 747., 770., 792., 816., 840., 865., 890., 916., 944., 1000., 1090., 1188., 1296., 1496.,
     ...
     2.05e8, 3.07e8, 4.10e8, 5.01e8, 6.13e8, 7.08e8, 8.18e8, 1.00e9],dtype=np.float32)
YKSHFG1 = np.array(
    [0.00, 3.39e-5, 6.77e-5, 1.00e-4, 1.32e-4, 1.63e-4, 1.92e-4, 2.21e-4, 2.48e-4, 2.75e-4, 3.00e-4, 3.25e-4,
     ...
     7.50e-5, 7.72e-5, 7.88e-5, 8.03e-5, 8.14e-5, 8.25e-5, 8.40e-5],dtype=np.float32)
...
Z6TG1 = np.array(
    [298., 178., 85.2, 47.5, 26.3, 12.2, 7.06, 4.45, 3.06, 2.82, 2.89, 2.99, 3.08, 3.13, 3.18, 3.25, 3.31, 3.39,
     3.44, 3.49, 3.52, 3.54, 3.55, 3.57, 3.57],dtype=np.float32)
Z9TG1 = np.array(
...
EIN = np.array([-0.0539, 0.0539, -0.0783, 0.0783, -0.1126, 0.1126, -0.1588, 0.1588, 0.3176, 0.4764, 11.5, 11.63, 11.88, 12.13,
...
YEL1 = [1.4945, 1.80, 2.25, 2.63, 3.20, 4.15, 5.10, 7.05, 8.90, 11.1,
